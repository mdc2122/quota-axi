...
An OmniRoute model row names every active `seat:*` scope in `quotaScopes` - each seat is an independent Cursor account pool the model can draw on - and takes the most constrained seat as its `effective` evidence, the conservative bound that never overstates headroom.
