...
quota-axi reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Z.AI, Alibaba, OpenCode Go, and Antigravity (`agy`) quota windows, plus Cursor seats hosted on a configured OmniRoute gateway, in one [AXI](https://axi.md)-shaped call.
...
- **Local first** - quota and auth reports run on the machine that holds the credentials; their network calls go to first-party provider endpoints, never a third-party relay. The one exception is the opt-in `omniroute` provider, which reads the configured OmniRoute gateway's own usage API because the seat credentials live on the gateway host, not locally.
...
attention[5]{provider,scope,kind,detail,remedy}:
  ...
  omniroute,all,auth_required,OMNIROUTE_API_KEY not set (needs an OmniRoute API key with manage scope),none
help[1]:
...
auth[18]{provider,source,path,status,error}:
  ...
  omniroute,omniroute-api,"http://127.0.0.1:20128",missing,OMNIROUTE_API_KEY not set (needs an OmniRoute API key with manage scope)
help[1]:
...
| Flag                                                                                     | Description                                                               |
| ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `--provider claude,codex,cursor,copilot,grok,kimi,zai,agy,alibaba,opencode-go,omniroute` | Scope providers                                                           |
...
| OmniRoute              | Opt-in provider that reads the configured OmniRoute gateway's own management API (`GET /api/providers` for the connection inventory, then `GET /api/usage/{connectionId}` per seat - the same read its dashboard's quota page makes). Each active `cursor` or `cursor-api` connection is an independent Cursor seat reported as its own `seat:<slug>` scope, and the seat's `Total`, `Auto + Composer`, and `API` quota keys jointly bound it, so effective remaining is the minimum across them. Dollar-denominated quotas also carry `spentUsd`/`limitUsd`; quotas with a `resetAt` report `monthly` windows, but no cycle start or duration is invented, so pace stays `unknown`. An unrecognized quota key or a failed seat usage read stays unresolved (`partial` semantics) rather than folding into a bound. An HTTP 401/403 retires the cache and reports `auth_required`, HTTP 429 reports `rate_limited` with `retryAfter`, an unreachable gateway serves the stale snapshot, and a reachable gateway with no Cursor seats is a fresh empty reading that clears the cache. |

### Model catalog and `models`

`quota-axi models [--intelligence high|medium|low] [--sort runway] [--provider ...] [--json|--full]` joins a reviewed catalog of native Claude, Codex, Grok, Kimi, and OmniRoute-routed Cursor models to the provider's effective quota evidence. It queries those five catalog-backed providers by default and accepts only those providers in an explicit models scope. Cursor IDE and Copilot remain excluded because their hosted model availability is plan-dependent; Copilot's quota relationships are also currently unknown. Z.AI, Alibaba, OpenCode Go, and Antigravity report quota but have no reviewed catalog entries yet, so they are not `models` providers either.
...
An OmniRoute model row names every active `seat:*` scope in `quotaScopes` - each seat is an independent Cursor account pool the model can draw on - and takes the usable seat with the most remaining quota as its `effective` evidence. If no seat has known remaining quota, `effective` is omitted.
...
| OmniRoute      | Environment only: `OMNIROUTE_API_KEY` (an OmniRoute API key with the `manage` scope) plus `OMNIROUTE_BASE_URL` (default `http://127.0.0.1:20128`); no credential files are read                                                                                                                                                                                                                                                                                              |
...
- The `omniroute` provider is opt-in and configured only through the environment: `OMNIROUTE_BASE_URL` (default `http://127.0.0.1:20128`) and `OMNIROUTE_API_KEY`, an OmniRoute API key with the `manage` scope. Without a usable key it reports `auth_required` and sends nothing. It is the one provider whose calls do not go to a first-party endpoint: the Cursor seat credentials live on the gateway host, so the only truthful reading is the gateway's own usage API.
- It sends two read-only `GET`s per refresh - `/api/providers` for the connection inventory (metadata only) and `/api/usage/{connectionId}` for each active `cursor` or `cursor-api` seat, the same call the gateway dashboard's quota page makes. Any usage refresh the usage route performs is OmniRoute's own behavior, identical to what its dashboard triggers; quota-axi never writes to the gateway, never reads seat credential material, and never asks the gateway to mint or rotate anything, so OmniRoute has no delegated refresh.
- Each seat gets one `seat:<slug>` scope; slugs prefer the connection name or email, fall back to the connection id, and suffix collisions deterministically in listed order. A seat whose usage read fails still appears with an `unknown` usage window rather than disappearing from the pool view.
- A definitive rejection (HTTP 401/403) retires the cached snapshot and reports `auth_required`; HTTP 429 reports `rate_limited` with `retryAfter`; transport failures serve the stale snapshot. A reachable gateway with no Cursor seats is a fresh empty reading that clears the cache. The API key is sent only as the bearer of these two requests and is never logged, rendered, cached, or leaked into error text.
...
| Cursor, GitHub Copilot, Kimi, Z.AI, Alibaba, OpenCode Go, Antigravity, OmniRoute | none                              | read-only; see the per-provider notes below           |
...
- It sends credential values only to the provider request they authenticate; the OmniRoute API key goes only to the configured gateway's own management API.
